...
public class ActivityFactory {
	 ...
	 */
	public static int produce (Vector<HashMap<String, Object>> tickets, DBHelper dbHelper) throws PomodroidException {
		int ticketsNumber = 0;
		final String autogen = "Autogenerated value"; 
		for (HashMap<String, Object> ticket : tickets) {
			
			if (isEmpty(ticket.get("summary").toString()))      ticket.put("summary", autogen);
			if (isEmpty(ticket.get("description").toString()))  ticket.put("description", autogen);
			if (isEmpty(ticket.get("priority").toString()))     ticket.put("priority", autogen);
			if (isEmpty(ticket.get("reporter").toString()))     ticket.put("priority", autogen);
			if (isEmpty(ticket.get("type").toString()))         ticket.put("priority", autogen);
